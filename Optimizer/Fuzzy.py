...
class Fuzzy:
    ...
    def update(self, mc, network, time_stem, check_func=check_request, self_charge_func=self_charge):
        ...
        for index, node in enumerate(checked_request_list):
            node_estimate = estimate(node.energy, node.energy_max, euclidean(node.location, mc.current), D_max,
                                     node.critical_density, CN_max, node.avg_energy, ECR_max)
        ...
        # print("choosen node: e={} max_e={} d={} dmax={} CN={} CN_max={} avg={} max_avg={}".format(node.energy,
        # node.energy_max, euclidean(node.location, mc.current), D_max, node.critical_density, CN_max, node.avg_energy,
        # ECR_max))
        return pos, time
